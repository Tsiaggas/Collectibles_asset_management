...
// Ολόκληρος ο teamNameMap από το άλλο function.
// Είναι σημαντικό να είναι ο ίδιος για συνέπεια.
...
function normalizeTeamName(name: string | null | undefined): string | undefined {
  ...
  // Πρώτα, παίρνουμε την αντιστοίχιση από τον χάρτη
  const lowerCaseName = name.trim().toLowerCase();
  const fromMap = teamNameMap[lowerCaseName];
  if (fromMap) {
    return fromMap;
  }
  // Αν δεν υπάρχει, απλά επιστρέφουμε το όνομα μετά από trim
  return name.trim();
}

const BATCH_SIZE = 100; // Πόσες κάρτες θα παίρνουμε σε κάθε πέρασμα
...
serve(async (_req) => {
  try {
    ...
    console.log("Starting team name cleanup process...");

    while (hasMore) {
      const from = page * BATCH_SIZE;
      const to = from + BATCH_SIZE - 1;

      console.log(`Fetching cards from index ${from} to ${to}...`);
      ...
      if (updates.length > 0) {
        console.log(`Found ${updates.length} cards to update in this batch. Updating...`);
        ...
        if (updateError) {
          ...
          // Αποφασίζουμε αν θέλουμε να σταματήσουμε ή να συνεχίσουμε
          // Για τώρα, συνεχίζουμε στην επόμενη σελίδα
        } else {
    ...
    const summary = `Cleanup complete. Processed ${processedCount} cards. Updated ${updatedCount} team names.`;
    console.log(summary);
    ...
    return new Response(JSON.stringify({ error: 'Failed to run cleanup', details: e.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
